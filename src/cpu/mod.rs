...
impl Cpu {
    ...
    pub fn pipeline_step(&mut self, inject_nop: bool) {
        ...
        let if_status = self.instruction_fetch(inject_nop);
    ...
    fn instruction_fetch(&mut self, inject_nop: bool) -> StageStatus<IfIdRegister> {
        instruction_fetch::execute(self, inject_nop)
    }
    ...
    fn memory_access(&mut self, ex_mem_reg: ExMemRegister) -> StageStatus<MemWbRegister> {
        memory_access::execute(self, ex_mem_reg)
    }
